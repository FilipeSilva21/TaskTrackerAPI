...
public class UserController {
    ...
    @PostMapping("/login")
    public ResponseEntity <Map<String, String>> login (@RequestBody CreateUserDTO userDTO){

        User user = userService.findUserByEmail(userDTO.email());

        if (user != null && passwordEncoder.matches(user.getPassword(), userDTO.password())){

            String token = tokenGenerator.generateToken(user.getName());
